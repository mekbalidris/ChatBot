...
export const getRoomsController = async (request, response) => {
    try {
        ...
        return response.status(200).json(await Room.find({ _id: { $in: user.rooms } }));
    } catch (error) {
...
export const postRoomController = async (request, response) => {
    try {
        ...
        await user.save();
        return response.status(201).json({ id: user.rooms[user.rooms.length - 1], msg:"Room created successfully"});
...
export const postMessageController = async (request, response) => {
    const { body: { isFromBot, content },params: { id } } = request;
    try {
        if (isFromBot === undefined || !content) return response.status(400).json({ msg: "Missing required fields: id, content, and isFromBot" });
...
export const getRoomController = async (request, response) => {
    ...
    try {
        const room = await Room.findOne({ _id: id });
        if (!room) return response.status(404).send({ msg: "The Room Does not Exist" });
